    ...
    <div class="col-lg-12">
        <div class="card card-default">
            ...
            @endforeach
                    ...
                    <input type="text" class="form-control" id="txt_first" name="point_earned" readonly>
					</div>--}}
					<div class="form-group">
                    <label for="exampleInputPassword1">current closing Balance :</label>
                ...
                <div class="col-md-6"> 
                <div class="form-group">
                    <label for="exampleInputPassword1">Monthly Bonus Earned :</label>
    ...
    $(document).ready(function() {
      $(document).on('click', '.view_point_button', function(e){
        e.preventDefault();
          ...
          var txt_first = $('#txt_first').val('0');
          var txt_sec = $('#txt_sec').val('0');
          var addpoint = $('#addpoint').val('0');
          var bonus = $('#bonus').val('0');
		  var close_bal = $('#close_bal').val('0');
		 ...
		 txt_first='';
		  ...
		  url: "{{ url('./point/ajax') }}"+"/"+email+"/"+ac_date,
                  ...
                  success:function(data) {
					  //console.log(data.bonus_point);

                    if(data){
         
					
					var b1 = data.points;
                    var b2 = data.bonus_point;
                    var b3 = data.opening_balance;
                    var b4 = data.Order_point;
					
					var opening_bal = data.opening_balance[0].closing_bal;
					

					$('#txt_sec').val(b4.length > 0 ? data.Order_point : '0');

					$('#addpoint').val(b1.length > 0 ? data.points[0].points : '0');	

                    $('#bonus').val(b2.length > 0 ? data.bonus_point[0].bonus_point : '0');
					
					
					var openingbal = parseInt($('#addpoint').val())+parseInt($('#bonus').val())+parseInt(opening_bal);
					
					console.log(openingbal);
					
					$('#txt_first').val(b3.length > 0 ?  openingbal : '0');
					

					var total = parseInt($('#txt_first').val())-parseInt($('#txt_sec').val());

                    
					
					$('#close_bal').val(data.closing_bal[0].closing_bal);
